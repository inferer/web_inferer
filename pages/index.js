...
export default function Home() {
  const [state, setState] = useState({ renderState: true, defaultOptions: { animationData } })
  useEffect(() => {
    setTimeout(() => {
      setState({ renderState: false })
    }, 2000)
  }, [])
  ...
  return (
    state.renderState ?
      (<div className='loadingContainer'>
        <div className="myLoading" style={{ height: state.defaultOptions.animationData.h / 2, width: state.defaultOptions.animationData.w / 2 }}>
        <Lottie
          options={state.defaultOptions}
          height={state.defaultOptions.animationData.h / 2}
          width={state.defaultOptions.animationData.w / 2}
        />
      </div>
      </div>)
