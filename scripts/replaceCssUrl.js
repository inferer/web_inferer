      ...
      fs.readdir(filePath, function (err, files) {
        ...
        files.forEach((filename) => {
          ...
          fs.stat(filedir, (eror, stats) => {
            ...
            if (isFile) {
              if (filedir.indexOf('DS_Store') <= 0 && filedir.indexOf('index.css') > -1) {
                // arr.push(filedir)
                const cssFile = path.resolve(__dirname, '..', filedir);
                console.log(cssFile)
                const cssStr = fs.readFileSync(cssFile);
                const replaeCssStr = cssStr.toString().replace(/url\(\'\//gi, "url('https://website-1315068501.cos.ap-nanjing.myqcloud.com/web_inferer/")
